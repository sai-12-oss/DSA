...
def Build_min_heap(nums):  # O(n)
    n = len(nums)
    for i in range(n // 2, -1, -1):
        min_heapify(nums, n, i)

def Extract_Min_from_Min_Heap(nums):  # O(logn)
    min_elem = nums[0]
    nums[0] = nums[-1]
...
def find_kth_element(nums, k):
    Build_min_heap(nums)  # O(n)
    ans = []
    for i in range(k):
        ans.append(Extract_Min_from_Min_Heap(nums))  # O(logn)
    return ans
...
def get_nearest_neighbors(E, estimate_vector):
    ...
    return [idx for dist, idx in find_kth_element(distance_map, 6)]
